...
namespace tweet {
	...
	proto::TweetOut Client::Tweet(const proto::TweetIn in)
	{
		...
		proto::TweetOut out;
        const auto status = stub_->Tweet(&client_context, in, &out);
		//if (!status.ok())
	...
	proto::FollowOut Client::Follow(const proto::FollowIn in)
	{
		grpc::ClientContext client_context;
		proto::FollowOut out;
		const auto status = stub_->Follow(&client_context, in, &out);
		//if (!status.ok())
	...
	proto::ShowOut Client::Show(const proto::ShowIn in)
	{
		grpc::ClientContext client_context;
		proto::ShowOut out;
		const auto status = stub_->Show(&client_context, in, &out);
		//if (!status.ok())
	...
	proto::LoginOut Client::Login(const proto::LoginIn in)
	{
		grpc::ClientContext client_context;
		proto::LoginOut out;
        const auto status = stub_->Login(&client_context, in, &out);
		//if (!status.ok())
	...
	proto::LogoutOut Client::Logout(const proto::LogoutIn in)
	{
		grpc::ClientContext client_context;
		proto::LogoutOut out;
        const auto status = stub_->Logout(&client_context, in, &out);
		//if (!status.ok())
	...
	proto::RegisterOut Client::Register(const proto::RegisterIn in)
	{
        ...
        const auto status = stub_->Register(&client_context, in, &out);
		//if (!status.ok())
